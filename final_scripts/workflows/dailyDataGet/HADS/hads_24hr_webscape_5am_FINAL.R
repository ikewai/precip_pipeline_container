...
#set up dirs:server
raw_page_wd<-"/home/mplucas/workflows/dailyDataGet/HADS/outFiles/raw"
parse_wd<-"/home/mplucas/workflows/dailyDataGet/HADS/outFiles/parse"
agg_daily_wd<-"/home/mplucas/workflows/dailyDataGet/HADS/outFiles/agg"
...
unagg_hads_monthly_filename<-paste0(format((Sys.Date()-1),"%Y_%m"),"_hads_all_data.csv")#dynamic filename that includes month year so when month is done new file is writen
...
all_hads_pc$obs_time<-(all_hads_pc$obs_time)-1 #minus 1 secound to put midnight obs in last day
tail(all_hads_pc)
...
rf_month_filename<-paste0("hads_daily_rf_5am","_",format((Sys.Date()-1),"%Y_%m"),".csv")#dynamic filename that includes month year so when month is done new file is written
