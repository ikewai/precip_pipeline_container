...
setwd("/home/mplucas/data/raw/hads/daily_agg")#server path
...
#append to all files
rf_filename<-"hads_all_daily_rf_5am.csv" 
if(max(as.numeric(files==rf_filename))>0){
	 write.table(hads_daily_rf_today,rf_filename, row.names=F,sep = ",", col.names = F, append = T)
      }else{
	 write.csv(hads_daily_rf_today,rf_filename, row.names=F)
	}
